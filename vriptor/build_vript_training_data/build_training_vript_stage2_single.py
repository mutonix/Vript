...
def build_training_vript(args):
    ...
    for cfile in tqdm.tqdm(gather_caption_files):
        ...
        for k in cap_keys:
            ...
            if voiceover and (("voice" in caption_content.lower() and "over" in caption_content.lower()) or "voicing" in caption_content.lower() or "narrat" in caption_content.lower() or "speak" in caption_content.lower() or "talk" in caption_content.lower() or "say" in caption_content.lower()):
                instruct = f"""Voiceover:"{voiceover}". Based on the video and voiceover, describe the video in detail (Duration: [0.0, {duration}]s, Sampling Rate: {sample_rate}s/frame)."""
                voice_over_num += 1
            else:
                instruct = f"Describe the video in detail (Duration: [0.0, {duration}]s, Sampling Rate: {sample_rate}s/frame)."
    ...
    json.dump(training_captions, open(os.path.join(args.output_dir, "vript_stage2_single.json"), "w"), indent=2)
