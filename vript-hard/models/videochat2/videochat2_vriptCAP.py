...
with open(vript_CAP_annotation_file, 'r') as f:
    vript_CAP_dataset = [json.loads(line) for line in f]

results = []    
system_prompt = "Carefully watch the video and pay attention to the cause and sequence of events, the detail and movement of objects, and the action and pose of persons."
question_prompt = "Describe this video in detail."
for data_sample in tqdm(vript_CAP_dataset):
    vid_path = f"{vript_CAP_video_path}/{data_sample['clip_id']}.mp4"
    num_frame = 16
    ...
    print(llm_message)
    results.append([data_sample['clip_id'], llm_message, data_sample['content'], data_sample])
